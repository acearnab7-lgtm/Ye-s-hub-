        ...
        <style>
            ...
            * { box-sizing: border-box; user-select: none; }
            ...
            header {
                ...
                background-color: var(--bg-secondary); padding: 10px 16px;
            ...
            .game-table {
                ...
                padding: 15px 10px; min-height: 380px; position: relative;
            }
            ...
            .insurance-overlay {
                ...
                background: rgba(15, 33, 46, 0.85); z-index: 100;
            ...
            .ins-yes { background: #00e701; color: #011d01; }
            .ins-no { background: var(--bg-card); color: white; }
            .ins-yes:disabled { opacity: 0.4; cursor: not-allowed; }

            .deck-shoe {
                position: absolute; top: 15px; right: 20px;
                font-size: 24px; background: #ffffff; border-radius: 4px;
                width: 32px; height: 20px; box-shadow: 0 3px 0 #b1bad3, 0 6px 0 #ffffff;
            }

            .hand-section { display: flex; flex-direction: column; align-items: center; width: 100%; }
            .score-bubble {
                background: #2f4553; color: #fff; padding: 4px 14px;
                border-radius: 12px; font-size: 13px; font-weight: 800; margin-bottom: 8px;
            }
            .cards-container { display: flex; justify-content: center; min-height: 105px; }

            .card {
                width: 65px; height: 95px; background: #ffffff; border-radius: 8px;
                display: flex; flex-direction: column; justify-content: space-between;
                padding: 6px; font-weight: 800; box-shadow: -4px 4px 10px rgba(0,0,0,0.4);
            ...
            .rule-banner {
                color: #2f4553; font-size: 11px; font-weight: 800;
                letter-spacing: 1.5px; text-align: center; line-height: 1.6;
            }

            .controls-panel { background: var(--bg-secondary); padding: 16px; }
            .btn-main-bet {
                width: 100%; background: var(--btn-primary); color: #ffffff; border: none;
                padding: 14px; border-radius: 4px; font-size: 16px; font-weight: bold;
            ...
            .bet-input-box {
                background: var(--bg-main); padding: 10px 14px; border-radius: 4px;
                display: flex; justify-content: space-between; align-items: center;
            ...
            .action-btn {
                background: var(--bg-card); color: #ffffff; border: none; padding: 14px;
            ...
            .icon-circle {
                background: var(--bg-main); width: 38px; height: 38px; border-radius: 50%;
                display: flex; justify-content: center; align-items: center; color: var(--text-muted); font-size: 18px;
            }
            .btn-fairness {
                background: var(--bg-card); color: var(--text-main); border: none;
                padding: 10px 16px; border-radius: 20px; font-weight: bold; display: flex; align-items: center; gap: 8px;
            }
            ...
            .orig-logo {
                background: #2f4553; width: 40px; height: 40px; border-radius: 6px;
                display: flex; justify-content: center; align-items: center; font-weight: bold; font-style: italic; margin-right: 12px;
            }
            .orig-text { flex-grow: 1; font-weight: bold; line-height: 1.3; }
            .orig-text small { color: var(--text-muted); font-weight: normal; font-size: 12px; }
            .orig-heart { background: var(--bg-main); padding: 10px; border-radius: 6px; color: var(--text-muted); }

            .save-game-row {
                background: var(--bg-card); border-radius: 8px; padding: 16px 12px;
                display: flex; justify-content: space-between; align-items: center; font-weight: bold; margin-bottom: 20px;
            }
            .save-badge {
                background: #2f4553; padding: 4px 8px; border-radius: 4px; font-size: 12px; margin-left: 8px; color: var(--text-muted);
            }
            .save-arrow { color: var(--text-muted); font-size: 18px; }

            .bottom-nav {
                display: flex; justify-content: space-around; padding: 15px 0;
                background: var(--bg-main); font-size: 11px; color: var(--text-muted); border-top: 1px solid #213743;
            }
    ...
    <body>
        ...
        <section class="controls-panel">
            ...
            <div class="originals-banner">
                ...
                <div class="orig-text">YE Originals<br><small>181.17K Followers</small></div>
        ...
        <nav class="bottom-nav">
            <div class="nav-item">🔍 Browse</div><div class="nav-item active">🎰 Casino</div>
            <div class="nav-item">⭐ For You</div><div class="nav-item">⚽ Sports</div><div class="nav-item">💬 Chat</div>
        </nav>
        ...
        <script>
            ...
            function dealHand() {
                ...
                }, 1300); // Wait for initial deal animations
            }
            ...
            function resolveInsurance(buy) {
                ...
                if (dScore === 21) {
                    renderTable(false);
                    if (insuranceBet > 0) {
                        balance += insuranceBet * 3; // Pays 2:1
                        setTimeout(() => alert("Dealer has Blackjack. Insurance pays!"), 500);
                    } else {
                        setTimeout(() => alert("Dealer has Blackjack!"), 500);
                    }
                    
                    if (calculateScore(playerHand) === 21) {
                        balance += currentBet; // Push main bet
                    }
                    gameOver = true;
                    updateBalance();
                    updateControls();
                    ...
                    if (insuranceBet > 0) {
                        setTimeout(() => alert("Dealer doesn't have Blackjack. Insurance lost."), 300);
                    }
                    checkInitialBlackjack();
                }
            }
            ...
            function checkInitialBlackjack() {
                if (calculateScore(playerHand) === 21) {
                    ...
                    updateControls(); // Turn on hit/stand buttons
                }
            }
            ...
            function hitAction() {
                ...
                renderTable(true);
                if (score >= 21) setTimeout(standAction, 1000);
            }
